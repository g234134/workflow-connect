# TICKET STATE · FP-G9-T3-p9-prod-ledger-gap-index-v1 · P9 prod ledger gap 索引
...
- Goal: 索引 P9 prod provider／ledger 缺口（≠ prod flip）。
- Scope:
  - MUST：新建 `docs/p9-prod-ledger-gap-index-v1.md`
  - MAY：`docs/index.md` 導航一行 · WORKFLOW_INDEX 一句（若有對應節）
- NonScope:
  - prod provider／registry flip
  - 碰 .env
- AllowedPaths:
  - `docs/p9-prod-ledger-gap-index-v1.md`
  - `04_Workflows/tickets/FP-G9-T3-p9-prod-ledger-gap-index-v1_state.md`（B/C/D_REPORT）
  - `docs/index.md`（MAY）
...
- AcceptanceCriteria:
  - AC-1：Landed vs Deferred／Blocked 表
  - AC-2：non_claims 含 ≠ prod flip
  - AC-3：rg 命中 prod|ledger|non_claims
...
- status_by_role:
  ...
  - scribe: done
- orch_notes: >-
  Batch-3 execute · FP-G9-T3-p9-prod-ledger-gap-index-v1 doc/spec 同輪 O→B→C→D 收口。
- reviewer_notes: >-
...
- changed_files:
  - docs/p9-prod-ledger-gap-index-v1.md（新建）
  - 04_Workflows/tickets/FP-G9-T3-p9-prod-ledger-gap-index-v1_state.md
- artifacts:
  - docs/p9-prod-ledger-gap-index-v1.md
- verification:
  - cmd: `rg "prod|ledger|sandbox|non_claims|blocked|provider" docs/p9-prod-ledger-gap-index-v1.md`
...
- docs_updates:
  ...
  - docs/index.md（MAY）
- progress_entry: |
  2026-07-10 · FP-G9-T3-p9-prod-ledger-gap-index-v1 done · P9 prod ledger gap 索引 · Reviewer accepted · branch_ai_closed ≠ Phase closure
