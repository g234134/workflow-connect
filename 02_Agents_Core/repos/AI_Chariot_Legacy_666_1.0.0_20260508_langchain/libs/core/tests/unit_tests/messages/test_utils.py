...
def test_get_buffer_string_xml_unicode_content() -> None:
    ...
    messages = [
        HumanMessage(content="雿末銝?"),  # Chinese: Hello World
        AIMessage(content="???怒??),  # Japanese: Hello
    ]
    result = get_buffer_string(messages, format="xml")
    expected = (
        '<message type="human">雿末銝?</message>\n'
        '<message type="ai">???怒??/message>'
    )
    assert result == expected
