...
async def test_dynamic_interrupt(async_checkpointer: BaseCheckpointSaver) -> None:
    ...
    assert [
        c
        async for c in tool_two.astream({"my_key": "value ?堆?", "market": "DE"}, thread2)
    ] == [
        {
            "__interrupt__": (
                Interrupt(
    ...
    assert [
        ...
        async for c in tool_two.astream(
            {"my_key": "value ?堆?", "market": "DE"}, thread1, durability="exit"
        )
    ] == [
        {
            "__interrupt__": (
    ...
    assert await tool_two.aget_state(thread1) == StateSnapshot(
        values={"my_key": "value ?堆?", "market": "DE"},
        next=("tool_two",),
        tasks=(
            PregelTask(
                AnyStr(),
...
async def test_partial_pending_checkpoint(
    ...
    tup = await tool_two.checkpointer.aget_tuple(thread1)
    assert await tool_two.aget_state(thread1) == StateSnapshot(
        values={"my_key": "value ?堆? one", "market": "DE"},
        next=("tool_two",),
        tasks=(
            PregelTask(
                AnyStr(),
