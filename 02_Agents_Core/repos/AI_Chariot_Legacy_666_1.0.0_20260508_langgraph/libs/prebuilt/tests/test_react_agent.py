...
def test__validate_messages():
    ...
    _validate_chat_history(
        [
            HumanMessage(content="What's the weather?"),
            AIMessage(content="The weather is sunny and 75簞F."),
        ]
    )
    ...
    _validate_chat_history(
        [
            ...
            ToolMessage(content="Sunny, 75簞F", tool_call_id="call1"),
            AIMessage(content="The weather is sunny and 75簞F."),
        ]
    )
    ...
    with pytest.raises(ValueError):
        _validate_chat_history(
            [
                ...
                ToolMessage(content="Sunny, 75簞F", tool_call_id="call1"),
                AIMessage(
                    content="The weather is sunny and 75簞F. Let me check the time."
                ),
            ]
        )
...
def test_react_agent_with_structured_response(version: str) -> None:
    ...
    def get_weather():
        """Get the weather"""
        return "The weather is sunny and 75簞F."
    ...
    for response_format in (WeatherResponse, ("Meow", WeatherResponse)):
        ...
        assert response["messages"][-2].content == "The weather is sunny and 75簞F."
    ...
    assert events == [
        ...
        {"post_model_hook": {"flag": True}},
        {
            "tools": {
                "messages": [
                    _AnyIdToolMessage(
                        content="The weather is sunny and 75簞F.",
                        name="get_weather",
                        tool_call_id="1",
                    ),
                ]
        ...
        },
        {
            "agent": {
                "messages": [
                    AIMessage(
                        content="What's the weather?-What's the weather?-The weather is sunny and 75簞F.",
                        additional_kwargs={},
                        response_metadata={},
                        id="3",
                        tool_calls=[
