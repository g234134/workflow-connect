...
class ChatModelIntegrationTests(ChatModelTests):
    ...
    def test_unicode_tool_call_integration(
        ...
        *,
        ...
        msgs = [
            HumanMessage(
                "Create a customer named '雿末???? (Hello Group) - a Chinese "
                "technology company"
            )
        ]
        ...
        if ai_msg.tool_calls:
            ...
            assert (
                "雿末" in customer_name
                or "雿? in customer_name
                or "憟? in customer_name
            ), f"Unicode characters not found in: {customer_name}"

        # Test with additional Unicode examples - Japanese
        msgs_jp = [
            HumanMessage(
                "Create a customer named '???怒?舀撘?蝷? (Hello Corporation) - a "
                "Japanese company"
            )
        ]
        ...
        if ai_msg_jp.tool_calls:
            ...
            assert (
                "???怒?? in customer_name_jp
                or "?芸?隡冗" in customer_name_jp
