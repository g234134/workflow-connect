...
class SandboxIntegrationTests(BaseStandardTests):
    ...
    def test_read_unicode_content(
        ...
        assert "銝?" in result.file_data["content"]
        assert "??邽赲迮?" in result.file_data["content"]
    ...
    def test_ls_unicode_filenames(
        ...
        sandbox_backend.write(f"{base_dir}/瘚??辣.txt", "content")
        sandbox_backend.write(f"{base_dir}/?訄邿郅.txt", "content")
        ...
        assert result.entries is not None
        paths = [entry["path"] for entry in result.entries]
        assert f"{base_dir}/瘚??辣.txt" in paths
        assert f"{base_dir}/?訄邿郅.txt" in paths
    ...
    def test_grep_unicode_pattern(
        ...
        sandbox_backend.write(
            ...
            "Hello 銝?\n??邽赲迮? 邾邽?\n瘚? pattern",  # noqa: RUF001
        )

        result = sandbox_backend.grep("銝?", path=base_dir)

        assert result.error is None
        assert result.matches is not None
        assert len(result.matches) == 1
        assert "銝?" in result.matches[0]["text"]
