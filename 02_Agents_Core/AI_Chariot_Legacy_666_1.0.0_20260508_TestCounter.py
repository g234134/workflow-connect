...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    async def test_data_access(self):
        from lightspeed.trex.logic.ogn.ogn.CounterDatabase import CounterDatabase
        test_file_name = "CounterTemplate.usda"
        usd_path = os.path.join(os.path.dirname(__file__), "usd", test_file_name)
        if not os.path.exists(usd_path):  # pragma: no cover
            self.assertTrue(False, f"{usd_path} not found for loading test")
        (result, error) = await ogts.load_test_file(usd_path)
        self.assertTrue(result, f'{error} on {usd_path}')
        test_node = og.Controller.node("/TestGraph/Template_lightspeed_trex_logic_Counter")
        database = CounterDatabase(test_node)
        ...
        self.assertTrue(test_node.get_attribute_exists("inputs:defaultValue"))
        attribute = test_node.get_attribute("inputs:defaultValue")
        self.assertTrue(attribute.is_valid())
        db_value = database.inputs.defaultValue
        database.inputs.defaultValue = db_value
        expected_value = 0.0
        ...
        database.inputs.increment = db_value
