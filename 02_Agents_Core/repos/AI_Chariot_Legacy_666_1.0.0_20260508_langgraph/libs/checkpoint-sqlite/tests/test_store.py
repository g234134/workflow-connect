...
def test_non_ascii(
    ...
    with create_vector_store(fake_embeddings, distance_type=distance_type) as store:
        store.put(("user_123", "memories"), "1", {"text": "餈銝剜?"})  # Chinese
        store.put(
            ("user_123", "memories"), "2", {"text": "???舀?祈??扼?"}
        )  # Japanese
        store.put(("user_123", "memories"), "3", {"text": "?湊探 ?筏?渥"})  # Korean
        store.put(("user_123", "memories"), "4", {"text": "衪?郋 ????郕邽邿"})  # Russian
        store.put(("user_123", "memories"), "5", {"text": "鄐能允 鄐啤?鄐詮? 鄐嫩?"})  # Hindi

        result1 = store.search(("user_123", "memories"), query="餈銝剜?")
        result2 = store.search(("user_123", "memories"), query="???舀?祈??扼?")
        result3 = store.search(("user_123", "memories"), query="?湊探 ?筏?渥")
        result4 = store.search(("user_123", "memories"), query="衪?郋 ????郕邽邿")
        result5 = store.search(("user_123", "memories"), query="鄐能允 鄐啤?鄐詮? 鄐嫩?")
