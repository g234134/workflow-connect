...
def test_graph_mermaid_special_chars(snapshot: SnapshotAssertion) -> None:
    graph = Graph(
        nodes={
            ...
            "撘憪?: Node(id="撘憪?, name="撘憪?, data=BaseModel, metadata=None),
            "蝏?": Node(id="蝏?", name="蝏?", data=BaseModel, metadata=None),
            "__end__": Node(
                id="__end__", name="__end__", data=BaseModel, metadata=None
            ),
        },
        edges=[
            Edge(source="__start__", target="撘憪?, data=None, conditional=False),
            Edge(source="撘憪?, target="蝏?", data=None, conditional=False),
            Edge(source="蝏?", target="__end__", data=None, conditional=False),
        ],
    )
    assert graph.draw_mermaid() == snapshot(name="mermaid")
